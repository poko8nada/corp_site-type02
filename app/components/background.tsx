...
export interface BgProps {
  background?: { patternClass: string } | { imageSrc: string };
...
export function Bg(props: BgProps) {
  ...
  const patternCls =
    background && 'patternClass' in background ? background.patternClass : undefined;
  ...
  return (
    <div data-bg class={wrapperCls}>
      {imageSrc && parallax && <ParallaxLayer speed={parallax.speed ?? 0.8} src={imageSrc} />}
      {imageSrc && !parallax && (
        <div
          aria-hidden='true'
      ...
      {patternCls && !parallax && (
        <div
          aria-hidden='true'
          class={`absolute inset-0 pointer-events-none ${patternCls ?? ''}`.trim()}
        />
      )}
